...
#elif(CLK_SOURCE_CONFIG == AHB_8)
#if(CLK_RCC_SOURCE == HSE)
	STK->LOAD = (HSE_CLOCK_VALUE_MEGA * 125)UL * copy_u32Delay_ms;
#elif(CLK_RCC_SOURCE ==HSI)
...
#elif(CLK_SOURCE_CONFIG == AHB_8)
#if(CLK_RCC_SOURCE == HSE)
	STK->LOAD = (HSE_CLOCK_VALUE_MEGA / 8)UL * copy_u32Delay_us;
#elif(CLK_RCC_SOURCE ==HSI)
...
#if(CLK_SOURCE_CONFIG == PROCCESOR_CLK_AHB)
#if(CLK_RCC_SOURCE == HSE)
	globalLoadValue = (HSE_CLOCK_VALUE_MEGA * 1000)UL * local_u8MilliTicks;
#elif(CLK_RCC_SOURCE == HSI)
	globalLoadValue = 16000UL * local_u8MilliTicks;
#endif
#elif(CLK_SOURCE_CONFIG == AHB_8)
#if(CLK_RCC_SOURCE == HSE)
	globalLoadValue = (HSE_CLOCK_VALUE_MEGA * 125)UL * local_u8MilliTicks;
#elif(CLK_RCC_SOURCE ==HSI)
